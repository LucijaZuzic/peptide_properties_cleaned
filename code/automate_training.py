...
def hyperparameter_tuning(model_name, test_number, train_and_validation_data, train_and_validation_labels, kfold_second, epochs, factor_NSA, mask_value = 2):
      ...
      for kernel in hyperparameter_kernel_size:   
        ...
        for pair in indices:   
            ...
            if "AP" in model_name and "SP" in model_name:
              model = new_model.amino_di_tri_model(input_shape = np.shape(train_data[3][0]), conv = CONV, numcells = numcells, kernel_size = kernel, lstm1 = LSTM, lstm2 = LSTM, dense = 2 * numcells, dropout = DROPOUT, lambda2 = LAMBDA, mask_value = mask_value)

            if "AP" in model_name and "SP" not in model_name:
              model = new_model.only_amino_di_tri_model(lstm1 = LSTM, lstm2 = LSTM, dense = 2 * numcells, dropout = DROPOUT, lambda2 = LAMBDA, mask_value = mask_value)
    ...
    # Choose correct model and instantiate model. 
    if "AP" in model_name and "SP" in model_name:
      model = new_model.amino_di_tri_model(input_shape = np.shape(train_and_validation_data[3][0]), conv = CONV, numcells = best_numcells, kernel_size = best_kernel, lstm1 = LSTM, lstm2 = LSTM, dense = 2 * best_numcells, dropout = DROPOUT, lambda2 = LAMBDA, mask_value = mask_value)

    if "AP" in model_name and "SP" not in model_name:
      model = new_model.only_amino_di_tri_model(lstm1 = LSTM, lstm2 = LSTM, dense = 2 * best_numcells, dropout = DROPOUT, lambda2 = LAMBDA, mask_value = mask_value)

    if "AP" not in model_name and "SP" in model_name:
      model = new_model.create_seq_model(input_shape = np.shape(train_and_validation_data[0]), conv1_filters = CONV, conv2_filters = CONV, conv_kernel_size = best_kernel, num_cells = best_numcells, dropout = DROPOUT, mask_value = mask_value)
...
def model_training(model_name, data, labels, best_numcells, best_kernel, epochs, factor_NSA, mask_value = 2):
    ...
    # Choose correct model and instantiate model. 
    if "AP" in model_name and "SP" in model_name:
      model = new_model.amino_di_tri_model(input_shape = np.shape(data[3][0]), conv = CONV, numcells = best_numcells, kernel_size = best_kernel, lstm1 = LSTM, lstm2 = LSTM, dense = 2 * best_numcells, dropout = DROPOUT, lambda2 = LAMBDA, mask_value = mask_value)

    if "AP" in model_name and "SP" not in model_name:
      model = new_model.only_amino_di_tri_model(lstm1 = LSTM, lstm2 = LSTM, dense = 2 * best_numcells, dropout = DROPOUT, lambda2 = LAMBDA, mask_value = mask_value)

    if "AP" not in model_name and "SP" in model_name:
      model = new_model.create_seq_model(input_shape = np.shape(data[0]), conv1_filters = CONV, conv2_filters = CONV, conv_kernel_size = best_kernel, num_cells = best_numcells, dropout = DROPOUT, mask_value = mask_value)

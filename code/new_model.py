...
def _create_seq_model(input_shape, conv1_filters = 64, conv2_filters = 64, conv_kernel_size = 6, num_cells = 64, dropout = 0.1, mask_value = 2):
    model_input = Input(shape = input_shape)
    mask = Masking(mask_value = mask_value)(model_input)
    
    if conv1_filters > 0:
        x = Conv1D(conv1_filters, conv_kernel_size, padding = 'same', kernel_initializer = 'he_normal')(mask)
...
def create_seq_model(input_shape, conv1_filters = 64, conv2_filters = 64, conv_kernel_size = 6, num_cells = 64, dropout = 0.1, mask_value = 2):
    model_input = Input(shape = input_shape, name = "input_1")
    mask = Masking(mask_value = mask_value)(model_input)
    
    if conv1_filters > 0:
        x = Conv1D(conv1_filters, conv_kernel_size, padding = 'same', kernel_initializer = 'he_normal', name = "conv1d_1")(mask)
